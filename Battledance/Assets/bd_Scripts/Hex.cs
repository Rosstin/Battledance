...
struct HexLayout
{
	...
	static public Point HexToPixel(HexLayout HexLayout, Hex h)
	{
		Orientation M = HexLayout.orientation;
		Point size = HexLayout.size;
		Point origin = HexLayout.origin;
	...
	static public FractionalHex PixelToHex(HexLayout HexLayout, Point p)
	{
		Orientation M = HexLayout.orientation;
		Point size = HexLayout.size;
		Point origin = HexLayout.origin;
	...
	static public Point HexCornerOffset(HexLayout HexLayout, int corner)
	{
		Orientation M = HexLayout.orientation;
		Point size = HexLayout.size;
	...
	static public List<Point> PolygonCorners(HexLayout HexLayout, Hex h)
	{
		List<Point> corners = new List<Point>{};
		Point center = HexLayout.HexToPixel(HexLayout, h);
		for (int i = 0; i < 6; i++)
		{
			Point offset = HexLayout.HexCornerOffset(HexLayout, i);
...
struct Tests
{
	//structs cannot contain parameterless constructors
	//public Tests()
	//{
	//}
	...
	static public void TestHexLayout()
	{
		Hex h = new Hex(3, 4, -7);
		HexLayout flat = new HexLayout(HexLayout.flat, new Point(10, 15), new Point(35, 71));
		Tests.EqualHex("HexLayout", h, FractionalHex.HexRound(HexLayout.PixelToHex(flat, HexLayout.HexToPixel(flat, h))));
		HexLayout pointy = new HexLayout(HexLayout.pointy, new Point(10, 15), new Point(35, 71));
		Tests.EqualHex("HexLayout", h, FractionalHex.HexRound(HexLayout.PixelToHex(pointy, HexLayout.HexToPixel(pointy, h))));
	}
